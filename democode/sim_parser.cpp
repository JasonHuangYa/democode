...
void RequestParam::get_params(std::map<std::string, std::string> &params){
	for(std::multimap<std::string, std::string>::iterator it = this->_params.begin(); it!= this->_params.end(); ++it ){
...
int RequestParam::parse_query_url(std::string &query_url) {
    ...
    DEBUG_PL(LOG_DEBUG,"start parse_query_url:%s\n", query_url.c_str());

    while(query_ss.good()) {
        std::string key_value;
        std::getline(query_ss, key_value, '&');
        DEBUG_PL(LOG_DEBUG,"get key_value:%s\n", key_value.c_str());
...
int RequestParam::parse_query_json(std::string &query_url) {
	Json::Reader reader;
	Json::Value root;
	DEBUG_PL(LOG_DEBUG, "post data:%s\n", query_url.c_str());
	if (reader.parse(query_url, root)) {
		Json::Value::Members mem = root.getMemberNames(); 
		for (auto iter = mem.begin(); iter != mem.end(); iter++){
			std::string key, value;
			key = *iter;
			int jstype= root[*iter].type();
			if (root[*iter].type() == Json::uintValue || jstype==1){
				value = to_str<int>(root[*iter].asInt());
			}else if (jstype == Json::realValue ){
				value = to_str<double>(root[*iter].asDouble());
			}else if (jstype == Json::objectValue || jstype == Json::arrayValue ){
				Json::FastWriter writer;
				value = writer.write(root[*iter]); 
			}else { 
				value = root[*iter].asString(); 
			}
			_params.insert(std::pair<std::string, std::string>(key, value));
		}
	}
...
int RequestLine::parse_request_url_params() {
    ...
    DEBUG_PL(LOG_DEBUG,"start parse params which request_url:%s\n", _request_url.c_str());
...
void RequestBody::get_params(std::map<std::string, std::string> &params){
    return _req_params.get_params(params);
}
...
int RequestBody::parse_multi_params() {
    for (size_t i = 0; i < _file_items.size(); i++) {
        ...
        DEBUG_PL(LOG_DEBUG,"request body add param name:%s, data:%s\n", 
...
void Request::get_params(std::map<std::string, std::string> &params){
    if (_line.get_method() == "GET") {
        _line.get_request_param().get_params(params);
    }
    if (_line.get_method() == "POST") {
        _body.get_params(params);
    }
}
...
int ss_on_header_field(http_parser *p, const char *buf, size_t len) {
    ...
    //DEBUG_PL(LOG_DEBUG,"GET field:%s\n", field.c_str());
    return 0;
}
...
int ss_on_header_value(http_parser *p, const char *buf, size_t len) {
    ...
    //DEBUG_PL(LOG_DEBUG,"GET value:%s\n", value.c_str());
...
int ss_on_headers_complete(http_parser *p) {
    ...
    if (req->_header_fields.size() != req->_header_values.size()) {
        DEBUG_PL(LOG_ERROR,"header field size:%u != value size:%u",
    ...
    //DEBUG_PL(LOG_DEBUG,"HEADERS COMPLETE! which field size:%u, value size:%u",
      //      req->_header_fields.size(), req->_header_values.size());
...
int ss_on_body(http_parser *p, const char *buf, size_t len) {
    ...
    //DEBUG_PL(LOG_DEBUG,"GET body len:%d", len);
    return 0;
}
...
int ss_on_message_complete(http_parser *p) {
	...
	if(ct_header == ""){
		ct_header = req->get_header("content-type");
	}
    // parse body params
	...
	}else {// if (ct_header == "application/json"){
		std::string *raw_str = req->get_body()->get_raw_string();
		if (raw_str->size()) {
			req->get_body()->get_req_params()->parse_query_json(*raw_str);
		}

	}
	req->_parse_part = PARSE_REQ_OVER;
	//DEBUG_PL(LOG_DEBUG,"msg COMPLETE!\n");
    return 0;
}
Request::Request() {
...
int Request::parse_request(const char *read_buffer, int read_size) {
    ...
    if (_total_req_size > MAX_REQ_SIZE) {
        DEBUG_PL(LOG_DEBUG,"TOO BIG REQUEST WE WILL REFUSE IT!");
        return -1;
    }
   DEBUG_PL(LOG_DEBUG,"read from client: size:%d, content:%s\n", read_size, read_buffer);
    ...
    if (nparsed != read_size) {
        ...
        DEBUG_PL(LOG_ERROR,"parse request error! msg:%s\n", err_msg.c_str());
        return -1;
    }
...
int Request::parase_cookie()
{
	string query_url = get_header("Cookie");

	if(query_url=="") query_url = get_header("cookie");
    ...
    while(query_ss.good()) {
        ...
        while(key_value_ss.good()) {
            ...
            std::getline(key_value_ss, key, '=');
			std::getline(key_value_ss, value, '=');
			DEBUG_PL(LOG_DEBUG,"key:%s, value:%s\n", key.c_str(), value.c_str());
			_cookiemap.insert(std::pair<std::string, std::string>(key, value));
        }
    }
    return 0;
}
...
int Response::gen_response(std::string &http_version, bool is_keepalive) {
   ...
   // DEBUG_PL(LOG_DEBUG,"gen response context:%s\n", _res_bytes.str().c_str());
    return 0;
}
...
void HttpContext::print_access_log(std::string &client_ip) {
    std::string http_method_ = this->_req->_line.get_method();
    std::string request_url = this->_req->_line.get_request_url();
    int cost_time = get_cost_time();
    DEBUG_PL(LOG_DEBUG,"access_log %s %s status_code:%d cost_time:%d us, body_size:%d, client_ip:%s\n",
            http_method_.c_str(), request_url.c_str(), _res->_code_msg.status_code,
