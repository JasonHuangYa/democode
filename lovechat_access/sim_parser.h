...
const static CodeMsg STATUS_BAD_REQUEST = {400, "Bad Request"};
const static CodeMsg STATUS_INTERNAL_SERVER_ERROR = {500, "Internal Server Error"};
...
class RequestParam {
    public:
		...
		void get_params(std::map<std::string, std::string> &params);

        /**
         * query_url : name=tom&age=3
         */
        int parse_query_url(std::string &query_url);
		int parse_query_json(std::string &query_url);
